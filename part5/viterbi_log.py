...
def backtracking(dp, words, tags, transitions_df):
    ...
    # From STOP to n
    for u in tags:
        score = log(dp[(n, u)]) + log(transitions_df.at[u, STOP])
        if score >= maximum:
            maximum = score
            argmax = u
    ...
    for j in range(n-1, 0, -1):
        ...
        for u in tags:
            score = log(dp[(j, u)]) + log(transitions_df.at[u, placeholder_argmax])
            if score >= maximum:
                maximum = score
                argmax = u
        results.append(argmax)
...
def forward(words, tags, emissions_df, transitions_df):
    ...
    for j in range(n):
        ...
        for u in tags:
            ...
            dp[(j+1, u)]=max([
                log(dp[(j,v)]) + log(b_u_o) + log(transitions_df.at[v,u]) for v in v_list
                ])
    # Stop 
    dp[(n+1, STOP)] = max([
        log(dp[(n,v)]) + log(transitions_df.at[v,STOP]) for v in tags
    ])
    return dp
